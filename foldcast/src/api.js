...
export async function api(path, body, raw = false, retry = true) {
  ...
  try {
    ...
    const response = await fetch(path, {
      ...
      ...(body === undefined ? {} : { method: 'POST', headers: { 'Content-Type': raw ? 'application/octet-stream' : 'application/json', 'X-FoldCast-Token': sessionToken }, body: raw ? body : JSON.stringify(body) }),
    });
...
function openDB() {
  if (!database) database = new Promise((resolve, reject) => {
    const request = indexedDB.open('foldcast-local-v1', 1);
