...
function stored(key, fallback) { try { return JSON.parse(localStorage.getItem(`foldcast.${key}`)) ?? fallback; } catch { return fallback; } }
function useSaved(key, fallback) { const [value, setValue] = useState(() => stored(key, fallback)); useEffect(() => { try { localStorage.setItem(`foldcast.${key}`, JSON.stringify(value)); } catch {} }, [key, value]); return [value, setValue]; }
