...
SPEC = importlib.util.spec_from_file_location("foldcast_server", Path(__file__).parents[1] / "server.py")
...
class HTTPTests(unittest.TestCase):
    ...
    def request(self, method, path, body=None, headers=None, token=True):
        ...
        if method == "POST":
            ...
            if token:
                merged["X-FoldCast-Token"] = self.app.token
    ...
    def test_host_origin_and_session_token_required(self):
        ...
        self.assertEqual(self.request("POST", "/api/recordings", headers={"X-FoldCast-Token": "wrong"})[0], 403)
