...
def package(destination):
    ...
    host_files = [
        (repo / "scripts" / "start-rivet", "start-rivet"),
        (repo / "scripts" / "install-rivet-termux.sh", "install-rivet-termux.sh"),
        (repo / "scripts" / "install-rivet-shortcut.sh", "install-rivet-shortcut.sh"),
    ]
    if any(not path.is_file() for path, _ in host_files):
        raise SystemExit("Termux launcher and installer files are required.")
    files = [(path, "rivet/" + path.relative_to(root).as_posix()) for path in runtime_files]
    files += [(path, "rivet/termux/" + archive_name) for path, archive_name in host_files]
        ...
        with gzip.GzipFile(filename="", mode="wb", fileobj=output, mtime=0) as zipped:
            with tarfile.open(mode="w", fileobj=zipped) as archive:
                for path, archive_name in files:
                    ...
                    info.mtime = 0
                    info.mode = 0o700 if path.name in {"launch.sh", "start-rivet", "install-rivet-termux.sh", "install-rivet-shortcut.sh"} else 0o600
    ...
    shutil.copyfile(repo / "scripts" / "install-rivet-termux.sh", standalone_installer)
